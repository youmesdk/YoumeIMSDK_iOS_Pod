...
- (void)setServerMode:(SERVER_MODE_t)serverMode{
    SetServerMode(serverMode);
}

- (void)setServerIpPort:(NSString*)ip port:(int)port{
    const char *mIP = [ip cStringUsingEncoding:NSASCIIStringEncoding];
    SetServerIpPort(mIP, port);
}
...
{
    IYouMeVoiceEngine::getInstance()->setToken( [token UTF8String ], time);
}
...
- (YouMeErrorCode)setMicbypassVolume:(unsigned int)vol
{
    return IYouMeVoiceEngine::getInstance()->setMicbypassVolume(vol);
}

- (int)getMicbypassVolume
{
    return IYouMeVoiceEngine::getInstance()->getMicbypassVolume();
}

- (long long)getBgmDuration
{
    return IYouMeVoiceEngine::getInstance()->getBgmDuration();
}
...
{
    return IYouMeVoiceEngine::getInstance()->setPcmCallbackEnable( self->imp, flag, bOutputToSpeaker, nOutputSampleRate, nOutputChannel);
}
